...
from .mamba_utils import StrideEmbed, create_block, RMSNorm, rms_norm_fn
...
def net_mamba_backbone(**kwargs):
    model = NetMambaBackbone(
        stride_size=4, embed_dim=256, depth=4,
        **kwargs)
    return model
...
def net_mamba_backbone_small(**kwargs):
    model = NetMambaBackbone(
        stride_size=4, embed_dim=192, depth=3,
        **kwargs)
    return model
...
def net_mamba_backbone_large(**kwargs):
    model = NetMambaBackbone(
        stride_size=4, embed_dim=384, depth=6,
        **kwargs)
    return model
